...
```{r}
# Import CSV
pa_raw <- read_csv(here("data/physical_activity.csv"))
```
...
# Subset data frame
pa_mini <- pa_raw %>% 
  select(!c(personal_id, household_id))  # Remove first two variables from data frame
```
...
```{r}
pa_clean4 <- pa_clean3 %>%
  mutate(sedentary_ap_m_wk = (sedentary_ap_s_day / 60) * 7,
         light_ap_m_wk = (light_ap_s_day / 60) * 7,
         mvpa_m_wk = (mvpa_s_day / 60) * 7) %>% 
  # select(!c(sedentary_ap_s_day, light_ap_s_day, mvpa_s_day))
  select(-ends_with("s_day"))  # A better way to drop the specified columns
```
...
```{r}
pa_clean5 <- pa_clean4 %>% 
  mutate(total_pa_m_wk = light_ap_m_wk + mvpa_m_wk)
```
...
pa_clean_final <- pa_clean5 %>% 
  mutate(met_recommendation = case_when(
    who_recommendation == "Yes" ~ TRUE,
    who_recommendation == "No" ~ FALSE
  ))
```
...
```{r}
pa_tabyl <- tabyl(pa_clean_final, status, met_recommendation)
```
...
```{r}
pa_histrogram <- ggplot(pa_clean_final) +
  aes(x = total_pa_m_wk, fill = status) +
  geom_histogram(bins = 30L) +
  scale_fill_hue(direction = 1) +
  labs(
    x = "Total Physical Activity (min/wk)",
    y = "Count",
    title = "Total Physical Activity"
  ) +
  theme_minimal() +
...
```{r}
pa_box_plot <- ggplot(pa_clean_final) +
  aes(x = total_pa_m_wk, y = status) +
